...
while True:
    try:
        ...
        url = f"https://www.rottentomatoes.com/tv/severance/s02?nocache={int(time.time())}"
        ...
        for index, market in markets_df.iterrows():
            ...
            def bucket_chance(exact_final_reviews):
            ...
            exact_final_reviews = 80
            prob = bucket_chance(exact_final_reviews)
            my_yes_odds_list.append(round(prob*100, 3))
        ...
        print(trade_df)
        #========================================================================================
        def calculate_bucket_chances(min_reviews, max_reviews, initial_reviews, initial_rating, bucket_threshold):
            results = []
            for exact_final_reviews in range(min_reviews, max_reviews + 1):
                additional_reviews = exact_final_reviews - initial_reviews
                #num_pos_reviews = round(initial_rating * initial_reviews)
                num_pos_reviews = initial_reviews
                def binomial_probability(n, k, p):
                    return math.comb(n, k) * (p ** k) * ((1 - p) ** (n - k))
                ...
                for future_pos_reviews in range(0, additional_reviews + 1):
                    final_pos_reviews = num_pos_reviews + future_pos_reviews
                    final_rating = final_pos_reviews / exact_final_reviews
                    probability = binomial_probability(additional_reviews, future_pos_reviews, initial_rating)
                    possible_outcomes.append((final_rating, probability))
        ...
        # Print results
        for final_reviews, probability in results:
          print(f"Final reviews: {final_reviews}, Probability: {probability:.4f}")


        review_counts, probabilities = zip(*results)

        # Plot the existing data
        plt.figure(figsize=(10, 6))
        plt.plot(review_counts, probabilities, 'b-', label=f'Probability of Exceeding {bucket_threshold * 100}% Rating')

        # Add horizontal line at 'yes book odds' for the given threshold
        threshold_row = trade_df[trade_df['threshold'] == bucket_threshold * 100]
        if not threshold_row.empty:
            yes_book_odds_value = threshold_row['yes book odds'].values[0] / 100
            plt.axhline(y=yes_book_odds_value, color='r', linestyle='--', label=f'Yes Book Odds at Threshold {bucket_threshold * 100}%')
        else:
            print(f"No data found for threshold {bucket_threshold}")


        plt.xlabel('Final Review Count')
        plt.ylabel('Probability')
        plt.title(f'Probability of Exceeding {bucket_threshold * 100}% Rating vs. Final Review Count')
        plt.legend()
