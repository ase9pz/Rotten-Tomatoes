...
# Configure logging
logging.basicConfig(filename='dog_man_trade_log.txt', level=logging.INFO, 
                    format='%(asctime)s - %(message)s')

# Configurable parameters
FINAL_REVIEW_ESTIMATE = 77  # Mean estimate for final review count
FINAL_REVIEW_STDDEV = 5     # Standard deviation for final reviews
TRADE_SIZE = 1              # Full size for simulated trades, easily adjustable
...
def calculate_future_bucket_chances(initial_reviews, initial_rating, bucket_threshold):
    ...
    # Define the range of possible final review counts.
    # We'll start at initial_reviews (as a minimum) and go up to FINAL_REVIEW_ESTIMATE + 3*FINAL_REVIEW_STDDEV.
    ...
    # Calculate unnormalized weights using the normal pdf (without the normalizing constant)
    # Since we'll renormalize them, the constant (1 / (sigma * sqrt(2pi))) is not necessary.
    ...
    for final_reviews, weight in zip(possible_final_reviews, weights):
        ...
        if additional_reviews < 0:
            continue  # Should not happen because min_final = initial_reviews
...
# The rest of your process_trades function would remain largely the same.
def process_trades(csv_file, output_file):
    df = pd.read_csv(csv_file)
    
    # Dictionary to keep track of positions per market
    ...
    with open(output_file, 'w') as f:
        for _, row in df.iterrows():
            ...
            # Initialize positions for the market if not already present
            ...
            try:
                ...
                # Optional: log no trade if none of the conditions match
...
# Example usage
process_trades('dog_man_market_data.csv', 'output.txt')
